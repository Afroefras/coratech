...
def compare_audios(mobile: np.array, stethos: np.array, sample_rate: int, offset: float=0.0) -> None:
    time_axis = np.linspace(0, len(stethos) / sample_rate, num=len(stethos))
    plt.figure(figsize=(12, 6))
    plt.plot(time_axis, stethos + offset, label="Estetoscopio", alpha=0.7, color="skyblue")
